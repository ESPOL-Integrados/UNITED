 ...
 //---------------------------------------------GRABAR ---------------------------------------------
        private void grabarToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (con.conectar())
                {
                   /** if (pbxProducto.BackgroundImage != null)
                    {
                        //clsImagen cimagen = new clsImagen();
                        MemoryStream ms = new MemoryStream();
                        Image.FromFile(txtBuscarImagen.Text).Save(ms, System.Drawing.Imaging.ImageFormat.Jpeg);

                        //oProducto_ent.IdImagen = Convert.ToInt32(txtCodigo.Text);

                        //cimagen.Descripcion = txtDescripcion.Text;

                        oProducto_ent.Imagen = ms.ToArray();
    ...
    //---------------------------------------------ACTUALIZAR --------------------------------------
        ...
        public void llenaComboBoxTipoProducto()
        {
            ...
            conexion.ConnectionString = "Data Source=.;Initial Catalog=SAV;Integrated Security=True";
             ...
             try
             {
                 if (!string.IsNullOrEmpty(selectedItemtp.ToString()) || !string.IsNullOrEmpty(selectedItemm.ToString()) 
                    || !string.IsNullOrEmpty(selectedItemb.ToString())
                     || !string.IsNullOrEmpty(selectedItemp.ToString()) || !string.IsNullOrEmpty(selectedIteme.ToString()))
                 {
                     RN_Producto con = new RN_Producto();
                     if (con.conectar())
                     {
